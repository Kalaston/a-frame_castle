...
AFRAME.registerComponent('teleport-listener', {
    ...
    init: function() {
        ...
        soundPool = SoundPool(
            utils.assetPath('assets/audio/portal.wav'),
            0.6,
            5
        );
            ...
            if (intersection.classList.contains('portal')) {
                ...
                if (this.el.sceneEl.dataset.isHome) {
                    localStorage.setItem(
                        'portalLocation',
                        JSON.stringify(intersection.getAttribute('position'))
                    );
                }
            } else {
                soundPool.play();
                ...
                if (this.el.sceneEl.dataset.isHome) {
                    localStorage.setItem(
                        'homeLocation',
                        JSON.stringify(cameraRig.getAttribute('position'))
                    );
                }
            }
        });
        ...
        hitEntity = this.hitEntity =
            el.components['teleport-controls'].hitEntity;
    ...
    tick: function() {
        ...
        if (!el.components['teleport-controls'].active) {
            return;
        }
        ...
        if (!intersection) {
            if (this.currentIntersection) {
                this.currentIntersection.emit('mouseleave');
            }
            this.currentIntersection = null;
            return;
        }

        if (intersection.object.el === this.currentIntersection) {
            return;
        }

        intersectingPortal = intersection.object.el.classList.contains(
            'portal'
        );
